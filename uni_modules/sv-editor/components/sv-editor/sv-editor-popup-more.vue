				...
				<view class="popup-image" v-if="toolName == 'image'">
					...
					<view class="popup-form-input">
						...
						<view class="form-thumbnail">
							<image class="form-thumbnail-item form-thumbnail-image"
								v-for="(item, index) in imageForm.file" :key="item.path || item.tempFilePath"
								:src="item.path || item.tempFilePath" @click="deleteImage(index)"></image>
						</view>
					</view>
				</view>
				<!-- 添加视频 -->
